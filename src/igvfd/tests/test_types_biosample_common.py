...
def test_biosample_rejects_dbxrefs(
    testapp, other_lab, human_donor, controlled_term_brain, biosample_type
):
    # Biosample no longer defines dbxrefs; archive identifiers are owned by Library,
    # SequenceFileSet, and MatrixFileSet. additionalProperties is false, so any value
    # is rejected regardless of form.
    endpoint, payload = _make_biosample_payload(other_lab, human_donor, controlled_term_brain, biosample_type)
    payload['dbxrefs'] = ['SRA:SRS12345']
